...
export const CACHED_SALE_WITNESS_V2_VERSION = 2;
export const CACHED_SALE_WITNESS_V2_LIMITS = Object.freeze({ scalar_utf8_bytes: 512, witness_utf8_bytes: 24_576 });
// Literal stored keys only. The three price-currency aliases are independent
// witnesses: no precedence, price binding, provider meaning or rights is inferred.
export const CACHED_SALE_WITNESS_V2_FIELDS = Object.freeze([
  ...
  'PriceCurrency', 'CurrentPriceCurrency', 'ClosePriceCurrency',
]);
...
function field(value, rootObject) {
  ...
  if (!rootObject) {
    ...
    if (!SCALARS.includes(kind) || kind === 'boolean' || text !== null || !Number.isInteger(bytes)
      || bytes <= CACHED_SALE_WITNESS_V2_LIMITS.scalar_utf8_bytes || bytes > 2_147_483_647) invalid('oversize_mismatch');
  } else if (state === 'scalar') {
    if (!SCALARS.includes(kind) || typeof text !== 'string' || text.length > CACHED_SALE_WITNESS_V2_LIMITS.scalar_utf8_bytes
      || !text.isWellFormed() || text.includes('\u0000') || !Number.isInteger(bytes) || Object.is(bytes, -0)
      || bytes < 0 || bytes > CACHED_SALE_WITNESS_V2_LIMITS.scalar_utf8_bytes || Buffer.byteLength(text, 'utf8') !== bytes) invalid('scalar_mismatch');
...
/** Dormant V2 admission of the exact installed SQL projection. Literal aliases
 * remain separate, including disagreements, blanks and unavailable states.
 * Stored JSONB observations do not establish provider bytes, history, units,
 * price currency or source rights. SQL NULL is an overflow sentinel, not absence.
 */
export function prepareCachedSaleWitnessV2(value) {
  if (value === null) invalid('witness_byte_limit');
  const item = closed(value, ['witness_version', 'root_state', 'root_json_type', 'fields']);
  if (item.witness_version !== CACHED_SALE_WITNESS_V2_VERSION) invalid('version');
  ...
  const supplied = closed(item.fields, CACHED_SALE_WITNESS_V2_FIELDS);
  const fields = Object.freeze(Object.fromEntries(CACHED_SALE_WITNESS_V2_FIELDS.map(key => [key, field(supplied[key], root === 'object')])));
  const result = Object.freeze({ ...item, fields });
  if (Buffer.byteLength(JSON.stringify(result), 'utf8') > CACHED_SALE_WITNESS_V2_LIMITS.witness_utf8_bytes) invalid('witness_byte_limit');
  return result;
}

// Fixed src alias and VALUES vocabulary; never select the complete raw payload,
// traverse arbitrary/private keys, or use caller-controlled SQL. Numeric scalars
// become PostgreSQL TEXT before Node parses the witness. Oversize fields retain
// only type/length; the whole-witness overflow sentinel must refuse atomically.
export const CACHED_SALE_WITNESS_V2_SQL = `(
  SELECT CASE WHEN octet_length(witness.body::text) <= ${CACHED_SALE_WITNESS_V2_LIMITS.witness_utf8_bytes}
    THEN witness.body ELSE NULL::jsonb END
  FROM (
    SELECT jsonb_build_object(
      'witness_version', ${CACHED_SALE_WITNESS_V2_VERSION},
      ...
      'fields', (SELECT jsonb_object_agg(keys.name, jsonb_build_object(
        'state', CASE WHEN jsonb_typeof(src.raw_payload) IS DISTINCT FROM 'object' THEN 'payload_unavailable'
          ...
          WHEN octet_length(src.raw_payload ->> keys.name) > ${CACHED_SALE_WITNESS_V2_LIMITS.scalar_utf8_bytes} THEN 'oversize' ELSE 'scalar' END,
        ...
        'value_text', CASE WHEN jsonb_typeof(src.raw_payload) = 'object'
          ...
          AND octet_length(src.raw_payload ->> keys.name) <= ${CACHED_SALE_WITNESS_V2_LIMITS.scalar_utf8_bytes}
      ...
      )) FROM (VALUES ${CACHED_SALE_WITNESS_V2_FIELDS.map(key => `('${key}')`).join(', ')}) AS keys(name))
    ) AS body
  ) AS witness
)`;
