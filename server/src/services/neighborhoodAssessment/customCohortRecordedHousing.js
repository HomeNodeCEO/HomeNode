...
import {
  CUSTOM_COHORT_RECORDED_HOUSING_CATEGORIES, CUSTOM_COHORT_RECORDED_HOUSING_STATES,
  CUSTOM_COHORT_RECORDED_HOUSING_ORIGINS, CUSTOM_COHORT_RECORDED_HOUSING_BASIS,
  CUSTOM_COHORT_RECORDED_HOUSING_LIMITS, CUSTOM_COHORT_RECORDED_HOUSING_LIMITATIONS,
  LEGACY, CAD_DESCRIPTIONS, LABELS, ALTERNATIVES, UNKNOWN,
  getCustomCohortRecordedHousingInterpretation,
} from './customCohortRecordedHousingProfiles.js';
export {
  CUSTOM_COHORT_RECORDED_HOUSING_CATEGORIES, CUSTOM_COHORT_RECORDED_HOUSING_STATES,
  CUSTOM_COHORT_RECORDED_HOUSING_ORIGINS, CUSTOM_COHORT_RECORDED_HOUSING_BASIS,
  CUSTOM_COHORT_RECORDED_HOUSING_LIMITS, CUSTOM_COHORT_RECORDED_HOUSING_LIMITATIONS,
  CUSTOM_COHORT_RECORDED_HOUSING_PROFILE, CUSTOM_COHORT_COMBINED_RECORDED_HOUSING_PROFILE,
  CUSTOM_COHORT_COUNTY_RECORDED_HOUSING_PROFILE, CUSTOM_COHORT_COMBINED_COUNTY_RECORDED_HOUSING_PROFILE,
  getCustomCohortRecordedHousingProfile, getCustomCohortRecordedHousingInterpretation,
} from './customCohortRecordedHousingProfiles.js';
...
export function buildCustomCohortRecordedHousing(args = {}) {
  ...
  const marker = get(input, 'recorded_housing_interpretation', true);
  const housingVersion = Object.hasOwn(input, 'recorded_housing_interpretation') ? 2 : 1;
  const interpretation = getCustomCohortRecordedHousingInterpretation(mapping, housingVersion);
  if (housingVersion === 2) {
    object(marker);
    check(Reflect.ownKeys(marker).length === 3 && ['id', 'revision', 'content_sha256']
      .every(key => get(marker, key) === interpretation.profile_ref[key]), 'interpretation_mismatch');
  }
  ...
  function aggregateParcel(row) {
    ...
    if (!row.counties.length || row.counties.some(value => value !== 'DALLAS'
      && !(housingVersion === 2 && value === 'DALLAS COUNTY'))) return resolution('unknown');
  ...
  const result = { housing_version: housingVersion, mapping_version: mapping, profile: interpretation.profile_ref,
