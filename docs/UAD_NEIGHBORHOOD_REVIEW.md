...
Positive sales require complete price observations and all three strictly positive price companions in ascending order. Known zero sales omit the price suggestions, while requiring explicit evidence that all three current price fields are empty. Missing membership/coverage is incomplete, not zero. Catalog overflow and overlong text block the whole candidate; values are never clamped, rounded or truncated to make them fit.
