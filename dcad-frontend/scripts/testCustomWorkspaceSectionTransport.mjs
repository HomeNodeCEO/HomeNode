...
for (const [version, groups, ackBytes] of [[5, 1024, 262144], [6, 2048, 524288]]) test(`v${version} serializes all ${groups + 1} IDs and admits its bounded ACK without changing old transport bounds`, async () => {
  const value = { workspace_version: version, active: { context_ref: {
  ...
  observation_period: { start_date: '2025-01-01', end_date: '2026-01-01' }, selection: { revision: 3,
    included_recorded_group_ids: [...Array.from({ length: groups }, (_, i) => `recorded-cad:${i.toString(16).padStart(64, '0')}`), 'discovery:unassigned'] } }, pending_capture: null };
  ...
  const huge = harness(); assert.throws(() => huge.transport.save('R-1', '17', { ...input(), value: { workspace_version: version,
    text: '€'.repeat(version === 6 ? 90000 : 45000) } }, { signal: signal() }));
  assert.equal(huge.calls.length, 0);
  if (version === 6) {
    const v5 = harness(); assert.throws(() => v5.transport.save('R-1', '17', { ...input(), value: { ...value, workspace_version: 5 } }, { signal: signal() }));
    assert.equal(v5.calls.length, 0);
  }
  const oversizedAck = harness(async () => json({ padding: 'x'.repeat(ackBytes) }));
