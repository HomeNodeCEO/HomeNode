...
async function canonicalState(pool, workfileId) {
  const [workfile, values, entities, assets, revisions, audit] = await Promise.all([
    pool.query(
...
function assertGuardFirst(trace) {
  ...
  const forbidden = trace.filter((statement) => (
    /appraisal\.(uad_field_values|uad_entities|uad_assets|uad_revisions|uad_audit_events)/.test(statement)
...
test("UAD canonical writers honor immutable lifecycle and signature state in a disposable PostgreSQL database", {
  ...
  try {
    ...
    const writers = [["saveUadSection", sectionWrite], ["applyUadCompletionSuggestions", completionWrite]];
    ...
    for (const [writerName, write] of writers) {
      for (const status of ["signed", "exported", "submitted", "cancelled"]) {
        await t.test(`${writerName} rejects ${status} before canonical reads and rolls back`, async () => {
          const workfile = await createWorkfileFixture(pool, identity, { status });
      ...
      await t.test(`${writerName} rejects signed_at even while status is mutable`, async () => {
        const workfile = await createWorkfileFixture(pool, identity, { status: "ready" });
        await pool.query(
    ...
    await t.test("save waits for an in-flight signature and then rejects it at unchanged revision", async () => {
      const workfile = await createWorkfileFixture(pool, identity, { status: "ready" });
      const before = await canonicalState(pool, workfile.workfileId);
      ...
      try {
        ...
        saving = sectionWrite(observed.pool, workfile);
        void saving.catch(() => {});
        await within(lockIssued.promise, "section save issued its workfile lock");
        await assertBlockedBy(pool, writerClient.processID, signer.processID, "section save");
        await insertSyntheticSignature(signer, identity, workfile, 1);
        await signer.query("COMMIT");
        signerTransaction = false;
        await assert.rejects(() => within(saving, "section save observed committed signature"), { message: LOCKED_ERROR });
        assertGuardFirst(observed.trace);
        assert.deepEqual(await canonicalState(pool, workfile.workfileId), before,
        ...
        signer.release();
        if (saving) await within(saving.catch(() => {}), "section save cleanup").catch(() => {});
        observed.forceRelease();
      }
    });

    await t.test("a sign-like decision observes the committed draft and revision after waiting for a save", async () => {
      const workfile = await createWorkfileFixture(pool, identity, { status: "ready" });
      const writerClient = await pool.connect();
      const signer = await pool.connect();
      let signerTransaction = false;
      ...
      try {
        ...
        saving = sectionWrite(observed.pool, workfile);
        void saving.catch(() => {});
        await within(writerLocked.promise, "section save acquired its workfile lock");
        signing = (async () => {
          await signer.query("BEGIN ISOLATION LEVEL READ COMMITTED");
          signerTransaction = true;
          const locked = await signer.query(
          ...
          assert.deepEqual(decision, { revision: 2, status: "draft" },
        ...
        const result = await within(saving, "section save committed before signer");
        ...
        assert.deepEqual(signDecision, { revision: 2, status: "draft" });
