...
import { createUadSectionPersistence } from "./editorPersistence.js";
...
export async function saveUadSection(
  ...
  try {
    ...
    const saved = await persistUadSectionWithClient(client, {
      workfileId,
      expectedRevision,
      saveReason,
      allowIncomplete,
    ...
    return {
      ...
      save_reason: saved.saveReason,
      saved_field_count: saved.normalizedCount,
      changed_field_count: saved.changedCount,
      values: saved.rows.map(responseValue),
      completion: completionFor(saved.rows, saved.entities, saved.assets),
    };
  } catch (error) {
