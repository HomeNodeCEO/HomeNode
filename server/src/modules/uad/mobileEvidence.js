...
async function finalizeBufferImport({
  ...
  await storage.putObject({ objectKey, contentType: input.content_type, body });
  const asset = await verifyUadAssetUpload(pool, storage, workfileId, assetId);
  await pool.query(
    `WITH actor AS (
       UPDATE appraisal.uad_assets
          SET created_by_user_id = COALESCE(created_by_user_id, $3), updated_at = now()
     ...
     ) SELECT $2, $3, $4, 'uad_asset', $1::text, $5::jsonb, $6::jsonb FROM actor`,
    [
      assetId,
      workfileId,
      actorUserId || null,
      ...
      JSON.stringify({ provenance_key: provenanceKey, provenance_value: provenanceValue }),
    ],
  );
...
export async function importUadMobilePhoto(pool, storage, workfileIdValue, photoIdValue, input = {}, actorUserId = null) {
  ...
  const photo = rows[0];
  const downloaded = await storage.getObject({ objectKey: photo.object_key });
  ...
  return finalizeBufferImport({
    pool, storage, workfileId, input: assetInput, body: downloaded.body,
