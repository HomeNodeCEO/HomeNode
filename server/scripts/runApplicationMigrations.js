...
const pool = new pg.Pool({
  ...
  max: 2,
  application_name: "homenode-application-migrations",
});

try {
  const uad = await applyUadMigrations(pool);
  const application = await applyMobileMigrations(pool);
  console.log(JSON.stringify({ uad, application }));
} finally {
  await pool.end();
}
