...
// v3 by relabeling individual rows: v3 needs its original installed metadata.
export function customCohortObservationMappingVersion(acquisition) {
  ...
  try {
    ...
    if (metadata?.reader_version !== 'local-capture-v3' || ![2, 3].includes(metadata.mapping_version)) throw new Error();
  } catch {
