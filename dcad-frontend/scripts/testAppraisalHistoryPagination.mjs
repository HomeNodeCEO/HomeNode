...
test('appraisal history uses bounded cursor pagination', () => {
  assert.match(apiSource, /URLSearchParams\(\{ limit: String\(page\.limit \|\| 25\) \}\)/);
  assert.match(apiSource, /if \(page\.cursor\) query\.set\('cursor', page\.cursor\)/);
  assert.match(apiSource, /next_cursor: string \| null/);
  assert.match(componentSource, /getPreviousAppraisalFiles\(accountId, \{ cursor: nextCursor \}\)/);
