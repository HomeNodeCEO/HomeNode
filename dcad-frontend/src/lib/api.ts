...
function assignmentWorkfileItemsPath(scope: AssignmentWorkfileItemScope) {
...
export async function getAssignmentWorkfileItems(
  scope: AssignmentWorkfileItemScope,
): Promise<AssignmentWorkfileItem[]> {
  const response = await fetchJSON<{ ok: true; items: AssignmentWorkfileItem[] }>(
    makeUrl(assignmentWorkfileItemsPath(scope)),
    { cache: 'no-store' },
  );
...
export async function uploadAssignmentWorkfileItem(
  ...
  const response = await fetchJSON<{ ok: true; item: AssignmentWorkfileItem }>(
    makeUrl(`${assignmentWorkfileItemsPath(scope)}/files`),
    {
      method: 'POST',
      headers: {
...
export async function createAssignmentWorkfileLink(
  ...
  const response = await fetchJSON<{ ok: true; item: AssignmentWorkfileItem }>(
    makeUrl(`${assignmentWorkfileItemsPath(scope)}/links`),
    {
      method: 'POST',
...
export async function downloadAssignmentWorkfileItem(
  ...
  const response = await fetchWithApplicationAuthentication(
    makeUrl(`${assignmentWorkfileItemsPath(scope)}/${encodeURIComponent(item.id)}/content`),
    { cache: 'no-store' },
  );
  if (!response.ok) {
...
export async function deleteAssignmentWorkfileItem(
  ...
  const response = await fetchWithApplicationAuthentication(
    makeUrl(`${assignmentWorkfileItemsPath(scope)}/${encodeURIComponent(itemId)}`),
    { method: 'DELETE' },
  );
  if (!response.ok) {
    const payload = await response.json().catch(() => null) as { error?: string } | null;
