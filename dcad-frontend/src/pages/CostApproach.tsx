...
export default function CostApproach() {
  ...
  return <CostApproachWorkspace key={`${request.propertyId}:${String(request.requestedFileId)}`} {...request} />;
}

function CostApproachWorkspace({ propertyId, requestedFileId }: ReturnType<typeof useAppraisalFileRequest>) {
  ...
  if (loading) return <main className="hn-app-shell p-8 text-slate-700">Loading Cost Approach...</main>;
