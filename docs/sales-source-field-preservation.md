...
`StandardStatus` is retained independently of the still-required `MlsStatus` column. For example, `MlsStatus=Closed` with `StandardStatus=" Pending "` preserves the disagreement and exact surrounding spaces; this does not change the importer's typed status or classify the record differently. A missing or blank `MlsStatus` is not supplied from `StandardStatus`. This lets a future authorized same-payload witness preserve an explicitly supplied consistency field, without changing its installed interpretation profile, activating a source mode, or altering old retained captures.
...
Application migration `20261018_sales_source_metadata.sql` adds only nullable `source_modified_at` and `source_system_name`. It supplies no ClosePrice, currency, or area-unit facts. This slice uses the existing raw payload and provenance columns and requires no schema migration. It does not activate the separately implemented combined Custom neighborhood mapping or expand historical source coverage.
