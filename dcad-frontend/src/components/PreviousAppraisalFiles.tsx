  ...
  return (
    <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-950">Previous Appraisal Files</h2>
          <p className="mt-1 text-xs leading-5 text-slate-600">
            Review Custom and UAD files together. Replication always preserves the original snapshot and lineage.
          </p>
        </div>
        <button className="btn btn-sm normal-case" disabled={loading} onClick={() => void load()} type="button">
          Refresh
        </button>
      </div>
      ...
      {loading ? (
        <p className="mt-4 text-sm text-slate-500">Loading prior appraisal files…</p>
      ) : orderedFiles.length === 0 ? (
        <p className="mt-4 text-sm text-slate-500">No Custom or UAD appraisal files have been created for this subject.</p>
      ) : (
        <div className="mt-4 space-y-4">
          {orderedFiles.map((file) => (
            <article className="rounded-xl border border-slate-200 bg-slate-50 p-4" key={file.id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <a className="btn btn-sm normal-case" href={file.view_url}>View File</a>
                  <button
                    className="btn btn-sm normal-case border-blue-700 bg-blue-700 text-white hover:bg-blue-800"
                    disabled={saving}
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                <div className="rounded-lg bg-white p-2"><dt className="text-slate-500">Effective date</dt><dd className="mt-0.5 font-semibold">{dateLabel(file.effective_date)}</dd></div>
                <div className="rounded-lg bg-white p-2"><dt className="text-slate-500">Condition / quality</dt><dd className="mt-0.5 font-semibold">{file.summary.condition_rating || '—'} / {file.summary.quality_rating || '—'}</dd></div>
                <div className="rounded-lg bg-white p-2"><dt className="text-slate-500">GLA</dt><dd className="mt-0.5 font-semibold">{numberLabel(file.summary.gross_living_area_sqft, ' sq. ft.')}</dd></div>
                <div className="rounded-lg bg-white p-2"><dt className="text-slate-500">Site / parcels</dt><dd className="mt-0.5 font-semibold">{numberLabel(file.summary.site_area_acres, ' ac.')} · {file.summary.parcel_count ?? '—'} parcel(s)</dd></div>
              </dl>
