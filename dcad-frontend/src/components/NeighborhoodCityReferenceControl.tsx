...
export default function NeighborhoodCityReferenceControl({
  map, subjectCity, onViewChange, onReturnToAnalysis,
}: {
  ...
  const defaultCity = catalog.cities.find(city => city.name.toLowerCase() === subjectCity?.trim().toLowerCase());
  const selected = catalog.cities.find(city => city.geoid === (chosen ?? defaultCity?.geoid));
  ...
  const show = async () => {
    ...
    try {
      ...
      onViewChange(true); setShown(selected.name);
    } catch (failure) {
  ...
  const hide = () => {
    ...
    setShown(null); setLoading(false); setError(null);
    onViewChange(false); onReturnToAnalysis();
  };

  return (
    <section aria-label="City boundary reference" className="mt-3 rounded-lg border border-amber-300 bg-gradient-to-r from-violet-50 to-amber-50 p-3">
      ...
      <p className="mt-2 text-xs text-slate-700">
        {shown ? `Showing ${shown}. ` : ''}Map reference only: showing a city does not add properties, change your drawn boundary, or recalculate statistics.
