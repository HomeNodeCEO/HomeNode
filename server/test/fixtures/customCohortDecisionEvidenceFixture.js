...
export async function decisionEvidenceFixture({ parcelCount = 2, missingCounty = false, saleOverrides = {}, extraTransactions = [], effectiveDate, assignmentFileId } = {}) {
  const f = customCohortRepositoryFixture({ effectiveDate, assignmentFileId }), t = f.state.input.target;
  ...
  const transactions = [transaction, ...extraTransactions.map(row => Object.fromEntries(Object.keys(transaction).map(key => [key, row[key]])))];
  ...
  const client = { release() { assert.fail('owner transaction'); }, async query(config) {
    ...
    switch (tag) {
      ...
      case 'transactions': rows = [{ ...transaction, record_type: 'closed_sale', sale_closing_date: '2024-03-01',
        ...
        source_year_built: 2001, source_garage_yn: false, source_housing_type: 'Single family', source_days_on_market: 0, data_quality_flags: [], ...saleOverrides }, ...extraTransactions]; break;
  ...
  const access = createTestCachedReadAccess({ target, scope: readScope, effective_date: subject.effective_date,
    ...
    source_revision: 'original-preparation-closure', transactions, links, legacy: [] } });
  ...
  return { f, client: f.client, store, scopeJson, accountIds, catalog, preview,
