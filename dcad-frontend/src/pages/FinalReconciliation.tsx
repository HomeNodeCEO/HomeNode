...
export default function FinalReconciliation() {
  ...
  const save = async () => {
    ...
    const editorKey = sessionStorage.getItem('homenode-editor-key') ||
      window.prompt('Enter the HomeNode editor key:', '')?.trim();
    if (!editorKey) return;
  ...
  if (loading) {
    return <main className="min-h-screen bg-slate-100 p-8 text-slate-700">Loading Final Reconciliation...</main>;
  }
  if (!detail || !draft) {
    return <main className="min-h-screen bg-slate-100 p-8 text-red-700">{message || 'Create an appraisal file before completing the Final Reconciliation.'}</main>;
  }
