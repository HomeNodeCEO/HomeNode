...
from scraper.dcad.field_completeness import assess_field_completeness
...
def insert_candidates(cursor, candidates: list[tuple[str, list[str]]]) -> None:
    cursor.execute(
        """
    ...
    execute_values(
        cursor,
        """
        INSERT INTO dcad_field_repair_candidates (account_id, flags)
        VALUES %s
        ON CONFLICT (account_id) DO UPDATE SET flags = EXCLUDED.flags
        """,
        candidates,
        page_size=2_000,
    )
...
def queue_candidates(cursor, candidates: list[tuple[str, list[str]]]) -> int:
    if not candidates:
        return 0
    ...
    cursor.execute(
        ...
        INSERT INTO app.dcad_field_repair_queue AS existing (
            account_id, status, attempts, next_attempt_at, reasons, updated_at
        )
        SELECT c.account_id, 'pending', 0, now(), c.flags, now()
            ...
            for row in cursor:
                ...
                if assessment.repair_required:
                    ...
                    if row.get("scrape_status") == "leased":
                        ...
                        candidates.append((str(row["account_id"]), flags))
